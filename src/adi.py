...
def adi_diffusion_3d_varD(u0_func, D_func, Lx, Ly, Lz, Nx, Ny, Nz,
    ...
    X, Y, Z = np.meshgrid(x, y, z, indexing='xy')  # shapes (Ny,Nx,Nz) but we'll reorder later
        ...
        for k in range(Nz):
            for j in range(Ny):
                ...
                for i in range(Nx):
                    ...
                    RHS[i] = u_ijk + 0.5*dt*( (Dcc[k,j,i]*lap_y) + (Dcc[k,j,i]*lap_z) + s )
            ...
            for i in range(Nx):
                ...
                for j in range(Ny):
                    ...
                    if i == 0:
                        ...
                        u_im = u_star[k,j-1+0, i-1] if False else u_star[k,j,i-1]  # simpler line; j index not used here
                        u_im = u_star[k,j,i-1] if i>0 else ghost_value('left', u_star[k,j,i])
                    if i == Nx-1:
                    ...
                    if k == Nz-1:
                        ...
                        u_kp = u_star[k+1,j,i]
                    lap_z = (u_km - 2.0*u_ijk + u_kp) / (dz*dz)
                    s = 0.0 if source_func is None else source_func(x[i], y[j], z[k], t + 0.5*dt)
                    RHS[j] = u_ijk + 0.5*dt*( (Dcc[k,j,i]*lap_x) + (Dcc[k,j,i]*lap_z) + s )
                ...
                if bc_type == 'dirichlet':
                    RHS[0] += rfac_y * (D_y[k,0,i] if Ny>1 else Dcc[k,0,i]) * bc_value
                    RHS[-1] += rfac_y * (D_y[k,-1,i] if Ny>1 else Dcc[k,-1,i]) * bc_value
            ...
            for i in range(Nx):
                ...
                for k in range(Nz):
                    ...
                    if j == Ny-1:
                        ...
                        u_jp = u_dbl[k,j+1,i]
                    lap_y = (u_jm - 2.0*u_ijk + u_jp) / (dy*dy)
                    s = 0.0 if source_func is None else source_func(x[i], y[j], z[k], t + dt)
                    RHS[k] = u_ijk + 0.5*dt*( (Dcc[k,j,i]*lap_x) + (Dcc[k,j,i]*lap_y) + s )
                ...
                if bc_type == 'dirichlet':
                    RHS[0] += rfac_z * (D_z[0,j,i] if Nz>1 else Dcc[0,j,i]) * bc_value
                    RHS[-1] += rfac_z * (D_z[-1,j,i] if Nz>1 else Dcc[-1,j,i]) * bc_value
